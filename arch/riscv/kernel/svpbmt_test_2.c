...
static int __init svpbmt_test_init(void)
{
	...
	pr_info("%s: Initializing SVPBMT test 2\n", TEST_NAME);
	...
	phys_addr = virt_to_phys(normal_buf);

	wc_buf = ioremap_wc(phys_addr, TEST_SIZE);
	if (!wc_buf) {
	...
	io_buf = ioremap(phys_addr, TEST_SIZE);
	if (!io_buf) {
	...
	if (svpbmt_supported) {
		...
		test_memory_performance(wc_buf, "Non-cacheable / WC");
