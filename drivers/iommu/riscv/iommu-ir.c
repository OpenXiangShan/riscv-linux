...
static bool riscv_iommu_ir_irq_has_vcpu_affinity(struct irq_data *data)
{
	struct riscv_iommu_ir_chip_data *chip_data = irq_data_get_irq_chip_data(data);

	return chip_data->vcpu_affinity;
}

static void riscv_iommu_ir_irq_set_vcpu_affinity_state(struct irq_data *data,
						       bool enabled)
{
	struct riscv_iommu_ir_chip_data *chip_data = irq_data_get_irq_chip_data(data);

	chip_data->vcpu_affinity = enabled;
}
...
static int riscv_iommu_ir_compose_parent_msi_msg(struct irq_data *data,
						 struct msi_msg *msg)
{
	for (; data; data = data->parent_data) {
		if (data->chip && data->chip->irq_compose_msi_msg) {
			data->chip->irq_compose_msi_msg(data, msg);
			return 0;
		}
	}
							...
							struct irq_data *data, phys_addr_t *addr)
{
	...
	BUG_ON(riscv_iommu_ir_compose_parent_msi_msg(data, &msg));
					   ...
					   const struct cpumask *dest, bool force)
{
	...
	if (riscv_iommu_ir_irq_has_vcpu_affinity(data) ||
	    old_config != domain->msitbl_config)
		return ret;
					  ...
					  struct riscv_iommu_ir_vcpu_info *vcpu_info)
{
	...
	if (domain->pgd_mode && !domain->stage2)
		riscv_iommu_ir_unmap_imsics(domain);
	...
	pte = &domain->msi_root[idx];
	riscv_iommu_ir_irq_set_msitbl_info(data, idx, domain->msitbl_config);
	riscv_iommu_ir_irq_set_vcpu_affinity_state(data, true);
...
static int riscv_iommu_ir_irq_set_vcpu_affinity(struct irq_data *data, void *arg)
{
	...
	if (!vcpu_info) {
		riscv_iommu_ir_irq_set_vcpu_affinity_state(data, false);
		riscv_iommu_ir_msitbl_unmap(domain, data, old_idx);
		return 0;
	}
						...
						void *arg)
{
	...
	for (i = 0; i < nr_irqs; i++) {
		...
		if (data->parent_data && msi_info && msi_info->desc)
			data->parent_data->common->msi_desc = msi_info->desc;
